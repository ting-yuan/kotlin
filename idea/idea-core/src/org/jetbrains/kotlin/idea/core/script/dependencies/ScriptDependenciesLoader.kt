...
abstract class ScriptDependenciesLoader(
    ...
    fun updateDependencies(file: VirtualFile, scriptDef: KotlinScriptDefinition) {
        if (cache[file] == null || fileModificationStamps[file] != file.modificationStamp) {
            fileModificationStamps.put(file, file.modificationStamp)

            loadDependencies(file, scriptDef)
        }
    }

    private val fileModificationStamps: SLRUMap<VirtualFile, Long> = SLRUMap(10, 10)
