...
private class DebuggerFieldSyntheticScope(val javaSyntheticPropertiesScope: JavaSyntheticPropertiesScope) : SyntheticScope.Default() {
    ...
    private fun isInEvaluator(location: LookupLocation) =
        (location as? KotlinLookupLocation)?.element?.containingKtFile?.suppressDiagnosticsInDebugMode ?: false
    ...
    private tailrec fun collectPropertiesWithParent(
        ...
        if (clazz is LazyJavaClassDescriptor) {
            collectJavaProperties(clazz, syntheticNames, consumer)
        } else if (clazz is JavaClassDescriptor) {
            error("Unsupported Java class type")
        } else {
            collectKotlinProperties(clazz, consumer)
        }
